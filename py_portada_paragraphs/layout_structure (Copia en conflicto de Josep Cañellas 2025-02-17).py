...
class AddingAreaInWrongColumnException(Exception):
    def __init__(self):
        super().__init__({'message': "Error trying add writing area to wrong column"})
        self.message = "Error trying add writing area to wrong column"
...
class AbstractSection:
    ...
    def get_single_sections_as_boxes(self, boxes=None, add_threshold=False, optimal_height=False):
        pass
...
class StructuredSection(AbstractSection):
    ...
    def get_single_sections_as_boxes(self, boxes=None, add_threshold=False, optimal_height=False):
        ...
        for section in self.sections:
            section.get_single_sections_as_boxes(boxes, add_threshold, optimal_height)
        return boxes
...
class MainLayout(StructuredSection):
    ...
    def get_single_sections_as_boxes(self, add_threshold=False, optimal_height=False):
        boxes = []
        return super().get_single_sections_as_boxes(boxes, add_threshold, optimal_height)
    ...
    def add_writing_area(self, writing_area_properties):
        ...
        if pos == -1:
            if self.can_be_inserted_a_column_for(writing_area_properties):
                ...
                # TODO ?????
                raise AddingAreaInWrongColumnException()
...
class SingleSection(AbstractSection):
    ...
    def _add_column_as_box(self, x1, y1, x2, y2, boxes, add_threshold=False):
        if type(add_threshold) is bool and add_threshold:
    ...
    def get_single_sections_as_boxes(self, boxes=[], add_threshold=False, optimal_height=False):
        if optimal_height is False:
            optimal_height = 2000
        x1, y1, x2, y2 = self.coordinates
        if y2 - y1 > optimal_height:
            h0 = y1
            h1 = y1
            id_wa = 0
            n = 0
            ...
            self._add_column_as_box(x1, y1, x2, y2, boxes, add_threshold)
